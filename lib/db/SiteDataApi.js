...
import { normalizeInternalSlug, processPostData } from '../utils/post'
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const normalizedFullSlug = normalizeInternalSlug(fullSlug)
  ...
  const findPost = () => {
    ...
    return (
      ...
      props.allPages.find(
        p =>
          p &&
          !p.type?.includes('Menu') &&
          normalizeInternalSlug(p.slug) === normalizedFullSlug
      ) ||
      // 2. UUID 匹配
